...
If gnu is down use mirror: https://raw.githubusercontent.com/d12frosted/elpa-mirror/master/gnu/
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun hbournis/org-capture-link ()
           ...
           (-contains?
            `(,clipboard
              ,(if (s-suffix? "/" clipboard)
                 ...
                 (concat clipboard "/")))
            it)
           (org-map-entries (lambda () (org-entry-get nil "URL")) "web_bookmarks" `(,hbournis/bookmarks-file)))
          (error "Link already exists.")))

    (defun hbournis/extract-host-from-clipboard ()
      (s-chop-prefix "www." (url-host (url-generic-parse-url (current-kill 0)))))

    (defun hbournis/org-tag-from-host-from-clipboard ()
      (s-replace-all '(("." . "_") ("-" . "_")) (hbournis/extract-host-from-clipboard)))
  ...
  (setq org-capture-templates `(
                                ("l" "Link" entry
                                 (file+function hbournis/bookmarks-file hbournis/org-capture-link)
                                 (file "~/.emacs.d/org-templates/bookmark.orgtemplate"))
                                ("L" "Private Link" entry
                                 (file+function hbournis/private-bookmarks-file hbournis/org-capture-link)
                                 (file "~/.emacs.d/org-templates/bookmark.orgtemplate"))
                                ("b" "Book" entry
